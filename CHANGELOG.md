...
- Distances read to whole miles for imperial accounts everywhere the web app shows one, so an odometer that read `99,419.27 mi` now reads `99,419 mi`. It matches the entry fields, which already took whole miles. Metric readings and stored values are unchanged.
...
- Metric fuel economy reads to two decimals, so `7.2` now reads `7.20 L/100km`. MPG still reads to one.
