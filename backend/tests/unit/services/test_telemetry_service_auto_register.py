...
class TestAutoRegisterNewParamInference:
    ...
    async def test_mqtt_style_registration_infers_percentage_class(self, db_session):
        ...
        # category is recomputed from the resolved (inferred) class, not left
        # at the "other" default a None class would produce.
        assert param.category == svc._classify_param("percentage")
