"""Defense-in-depth test for the fuel_station vendor-sync bypass (issue #69).

The fuel-record save path creates `address_book` entries with
`poi_category='fuel_station'` for newly-typed station names. These entries
...
class _SessionRecorder:
    ...
    def begin_nested(self):  # pragma: no cover - exercised when guard fails
        ...
        raise AssertionError("fuel_station entries must not enter the vendor-sync transaction")


@pytest.mark.asyncio
async def test_fuel_station_entry_skips_vendor_sync():
    """A poi_category='fuel_station' entry must short-circuit the helper."""
    entry = _FakeEntry(business_name="Costco Gas #42", poi_category="fuel_station")
    db = _SessionRecorder()
